...
function codenovo_portfolio_messages($messages)
{
    ...
    $messages['portfolios'] = array(
        ...
        9 => sprintf(
            __('Portfolio scheduled for: <strong>%1$s</strong>. <a target="_blank" href="%2$s">Preview portfolio</a>'),
...
function codenovo_portfolio()
{
    ...
    $item = 0;
    ?>
    <div id="cnfolioHolder" class="container cnfolio">
        ...
        <div class="slider">
            <ul class="slider__list slider__list_photo cnOurHolder" date_1="0">
                <?php if ($loop):
                    $i = 1;
                    while ($loop->have_posts()):
                        $loop->the_post();
                        ...
                        $portfolio_url = get_post_custom_values('_url');
                        $portfolio_url_tab = get_post_custom_values('_ntab');
                        $nTab = ($portfolio_url_tab[0]) ? 'target="_blank"' : '';
                        $url = (preg_match("/\b(?:https?|ftp):\/\/|www\.)/", $portfolio_url[0])) ? $portfolio_url[0] : 'http://' . $portfolio_url[0];
                ?>
                        <li class="slider__item slider_photo slider_photo_active item" data-id="id-<?php echo $i; ?>" data-type="<?php echo $cd_pf_class; ?>" data-portfolio-url="<?php echo $url; ?>" data-nTab="<?php echo $nTab; ?>" data-index="<?php echo $i++; ?>">
                            <img src="<?php echo $imgUrl[0]; ?>" alt="" class="slider__img pop-up-img">
                    ...
                    <li class="error-not-found">Sorry, no portfolio entries for now.</li>
                <?php endif; ?>
            </ul>
        </div>
        ...
        jQuery(document).ready(function($) {
            ...
            */
            ...
            $('#cnFilterOptions li a').click(function(e) {
                $('#slider-first_photo').trigger('click');
